...
class AnomalyDetection:
    ...
    def elliptic_envelope_analysis(self, seq_ids: Optional[List[str]] = None,
                                  contamination: float = 0.1,
                                  support_fraction: Optional[float] = None) -> Dict:
        """
    ...
    def ensemble_anomaly_detection(self, seq_ids: Optional[List[str]] = None,
        ...
        for method in methods:
            if method == 'isolation_forest':
                ...
                result = self.local_outlier_factor_analysis(seq_ids, contamination=contamination)
                individual_results[method] = result
                anomaly_votes += result['is_anomaly'].astype(int)
                decision_scores_ensemble += -result['lof_scores']  # LOF scores are negative
                
            elif method == 'one_class_svm':
                result = self.one_class_svm_analysis(seq_ids, nu=contamination)
                individual_results[method] = result
                anomaly_votes += result['is_anomaly'].astype(int)
                decision_scores_ensemble += -result['decision_scores']  # Make consistent
    ...
    def _calculate_local_densities(self, matrix: np.ndarray, n_neighbors: int) -> np.ndarray:
        ...
        local_densities = 1.0 / (mean_distances + 1e-10)  # Add small epsilon to avoid division by zero
        
        return local_densities
    
    def _calculate_anomaly_statistics(self, matrix: np.ndarray, is_anomaly: np.ndarray,
        ...
        if np.sum(anomaly_mask) > 0 and np.sum(normal_mask) > 0:
            ...
            stats['normal'] = {
                ...
                'sequences': [seq_ids[i] for i in range(len(seq_ids)) if normal_mask[i]]
            }
            ...
            stats['anomaly'] = {
                ...
                'sequences': [seq_ids[i] for i in range(len(seq_ids)) if anomaly_mask[i]]
            }
            
            # K-mer differences
            mean_diff = stats['anomaly']['mean_profile'] - stats['normal']['mean_profile']
            ...
            for i, kmer in enumerate(kmer_list):
                ...
                if stats['normal']['std_profile'][i] > 0:
                    ...
                    pooled_std = np.sqrt(
                        (stats['normal']['std_profile'][i]**2 + stats['anomaly']['std_profile'][i]**2) / 2
                    )
                    ...
                    discriminative_kmers.append({
                        ...
                        'fold_change': anomaly_mean / normal_mean if normal_mean > 0 else float('inf')
                    })
    ...
    def threshold_analysis(self, seq_ids: Optional[List[str]] = None,
    ...
    def cross_validation_analysis(self, seq_ids: Optional[List[str]] = None,
                                 method: str = 'isolation_forest',
                                 n_folds: int = 5) -> Dict:
        """
        ...
        for fold in range(n_folds):
            ...
            train_indices = np.concatenate([
                fold_indices[:start_idx],
                fold_indices[end_idx:]
            ])
        ...
        mean_anomaly_rate = np.mean([result['n_anomalies'] / len(result['test_indices']) 
                                   for result in cv_results])
        std_anomaly_rate = np.std([result['n_anomalies'] / len(result['test_indices']) 
                                 for result in cv_results])
