...
class IVOMAnalysis:
    ...
    def extract_variable_order_motifs(
        self, sequence: str
        ...
        for order in range(self.min_order, self.max_order + 1):
            ...
            if total_motifs > 0:
                order_distributions[order] = {
                    motif: count / total_motifs for motif, count in motif_counts.items()
                }
            else:
                order_distributions[order] = {}
    ...
    def calculate_interpolated_distribution(
        self, order_distributions: Dict[int, Dict[str, float]]
    ) -> Dict[str, float]:
        """
        ...
        for i, order in enumerate(range(self.min_order, self.max_order + 1)):
            ...
            weight = (
                self.interpolation_weights[i]
                if i < len(self.interpolation_weights)
                else 0.1
            )
    ...
    def build_background_model(self, reference_seqs: List[str]) -> Dict[str, float]:
        ...
        missing_seqs = [
            seq_id for seq_id in reference_seqs if seq_id not in self.profiler.sequences
        ]
        if missing_seqs:
        ...
        if total_freq > 0:
            background_dist = {
                motif: freq / total_freq for motif, freq in background_dist.items()
            }

        self.background_distribution = background_dist
        return background_dist

    def calculate_compositional_deviation(
        self, sequence: str, background_dist: Optional[Dict[str, float]] = None
    ) -> float:
        """
        ...
        if background_dist is None:
            raise ValueError(
                'No background distribution available. Run build_background_model first.'
            )
    ...
    def ivom_analysis(
        ...
        [
            self.profiler.sequences[seq_id] for seq_id in reference_seqs
        ]
        ...
        for seq_id in target_seqs:
            ...
            try:
                deviation_score = self.calculate_compositional_deviation(
                    sequence, background_dist
                )
                ...
                order_dists = self.extract_variable_order_motifs(sequence)
                motif_distributions[seq_id] = self.calculate_interpolated_distribution(
                    order_dists
                )
    ...
    def sliding_window_ivom_analysis(
        self,
        seq_id: str,
        ...
        **ivom_kwargs,
        ...
        if len(sequence) < window_size:
            raise ValueError('Sequence length smaller than window size')
        ...
        reference_windows = [windows[i] for i in range(0, len(windows), max(1, step))][
            :n_ref
        ]
        ...
        ivom_results = temp_ivom.ivom_analysis(
            reference_seqs=reference_windows, target_seqs=windows, **ivom_kwargs
        )
