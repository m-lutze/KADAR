...
class ClusteringAnalysis:
    ...
    def kmeans_clustering(
        ...
        calinski_harabasz = (
            calinski_harabasz_score(matrix_scaled, labels) if n_clusters > 1 else None
        )
        davies_bouldin = (
            davies_bouldin_score(matrix_scaled, labels) if n_clusters > 1 else None
        )
    ...
    def hierarchical_clustering(
        ...
        silhouette = (
            silhouette_score(matrix_scaled, labels) if n_clusters_found > 1 else None
        )
    ...
    def gaussian_mixture_clustering(
        ...
        # Calculate clustering metrics
        silhouette = (
            silhouette_score(matrix_scaled, labels) if n_components > 1 else None
        )
    ...
    def optimal_clusters_analysis(
        ...
        for k in cluster_range:
            ...
            metrics['calinski_harabasz_scores'].append(
                calinski_harabasz_score(matrix_scaled, labels)
            )
            metrics['davies_bouldin_scores'].append(
                davies_bouldin_score(matrix_scaled, labels)
            )

        # Find optimal number of clusters
        optimal_k = {}
        optimal_k['silhouette'] = cluster_range[np.argmax(metrics['silhouette_scores'])]
        optimal_k['calinski_harabasz'] = cluster_range[
            np.argmax(metrics['calinski_harabasz_scores'])
        ]
        optimal_k['davies_bouldin'] = cluster_range[
            np.argmin(metrics['davies_bouldin_scores'])
        ]
        ...
        if method == 'kmeans':
            ...
            if len(wcss_diff2) > 0:
                elbow_k = cluster_range[
                    np.argmax(wcss_diff2) + 2
                ]  # +2 due to double diff
        ...
        for label in unique_labels:
            ...
            if len(cluster_data) > 0:
                stats = {
                    ...
                    'intra_cluster_distance': self._calculate_intra_cluster_distance(
                        cluster_data
                    ),
                ...
                if len(cluster_data) > 1:
                            ...
                            characteristic_kmers.append(
                                {
                                    ...
                                    'coefficient_variation': kmer_std / abs(kmer_mean)
                                    if kmer_mean != 0
                                    else float('inf'),
                                }
                            )
                    ...
                    characteristic_kmers.sort(
                        key=lambda x: x['mean_frequency'], reverse=True
                    )
    ...
    def consensus_clustering(
        ...
        for k in range(2, min(8, n_samples)):
            clusterer = AgglomerativeClustering(
                n_clusters=k, linkage='precomputed', metric='precomputed'
            )
        ...
        final_clusterer = AgglomerativeClustering(
            n_clusters=best_k, linkage='precomputed', metric='precomputed'
        )
        ...
        for i in range(n_bootstrap):
            ...
            if np.sum(valid_mask) > 0:
                ari = adjusted_rand_score(
                    original_labels[valid_mask], mapped_labels[valid_mask]
                )
    ...
    def _calculate_jaccard_similarity(
        self, labels1: np.ndarray, labels2: np.ndarray
    ) -> float:
