...
class StatisticalAnalysis:
    ...
    def z_score_analysis(self, reference_seqs: List[str], target_seqs: List[str]) -> Dict:
        ...
        invalid_refs = [seq_id for seq_id in reference_seqs if seq_id not in all_seq_ids]
        invalid_targets = [seq_id for seq_id in target_seqs if seq_id not in all_seq_ids]
        ...
        for i, target_seq in enumerate(target_seqs):
            ...
            if np.sum(mask) > 1:  # Need at least 2 categories
                try:
                    ...
                    contributions = ((observed_counts[mask] - expected_counts[mask])**2 / 
    ...
    def kolmogorov_smirnov_analysis(self, reference_seqs: List[str], 
                                   target_seqs: List[str]) -> Dict:
        """
    ...
    def permutation_test(self, reference_seqs: List[str], target_seqs: List[str],
                        n_permutations: int = 1000, statistic: str = 'mean_difference') -> Dict:
        """
        ...
        if statistic == 'mean_difference':
            ...
            observed_stat = np.var(target_matrix, axis=0) / (np.var(ref_matrix, axis=0) + 1e-10)
        else:
        ...
        for _ in range(n_permutations):
            ...
            if statistic == 'mean_difference':
                ...
                perm_stat = np.var(perm_target, axis=0) / (np.var(perm_ref, axis=0) + 1e-10)
