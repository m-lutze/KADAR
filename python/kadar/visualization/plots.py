...
class ResultsVisualizer:
    ...
    def _plot_clustering(
        self, results: Dict, pca_results: Optional[Dict] = None, **kwargs
    ):
    ...
    def _plot_z_score(self, results: Dict, **kwargs):
        ...
        axes[0, 1].hist(
            composite_z_scores,
            bins=min(10, len(composite_z_scores)),
            alpha=0.7,
            edgecolor='black',
        )
        ...
        im = axes[1, 0].imshow(
            z_scores_plot.T, aspect='auto', cmap='RdBu_r', vmin=-3, vmax=3
        )
    ...
    def _plot_anomaly_detection(
        ...
        if pca_results is not None:
            ...
            axes[0, 0].scatter(
                components[:, 0], components[:, 1], c=colors, alpha=0.7, s=60
            )
    ...
    def _plot_chi_square(self, results: Dict, **kwargs):
        ...
        axes[0, 1].axhline(
            y=-np.log10(0.05), color='red', linestyle='--', label='p=0.05'
        )
        axes[0, 1].axhline(
            y=-np.log10(0.001), color='darkred', linestyle='--', label='p=0.001'
        )
        ...
        for _i, (chi2, log_p, seq_id) in enumerate(
            zip(
                chi2_stats[significant_mask],
    ...
    def create_summary_dashboard(self, all_results: Dict, pca_results: Dict):
        ...
        plt.figure(figsize=(20, 5 * ((n_methods + 1) // 2)))
        ...
        for method, results in all_results.items():
            ...
            if method in ['isolation_forest', 'lof', 'one_class_svm']:
                ...
                colors = ['red' if anomaly else 'blue' for anomaly in is_anomaly]
                ax.scatter(
                    components[:, 0], components[:, 1], c=colors, alpha=0.7, s=40
                )
                ax.set_title(
                ...
                ax.set_title(
                    f'{results["method"].title()} Clustering\n{n_clusters} clusters'
                )
        ...
        plt.tight_layout()
        plt.suptitle('Genome Island Detection Summary Dashboard', fontsize=16, y=1.02)
        plt.show()

    def plot_method_comparison(
        self, results_dict: Dict, ground_truth: Optional[List[bool]] = None
    ):
        """
        ...
        for i in range(n_methods):
            for j in range(n_methods):
                axes[0, 0].text(
                    j, i, f'{agreement_matrix[i, j]:.2f}', ha='center', va='center'
                )
    ...
    def _plot_method_overlap(self, method_predictions: Dict, ax):
        ...
        if n_methods == 2:
            # Two-way overlap
            pred1, pred2 = (
                method_predictions[methods[0]],
                method_predictions[methods[1]],
            )
    ...
    def _plot_performance_metrics(
        self, method_predictions: Dict, ground_truth: List[bool], ax
    ):
    ...
    def _plot_ivom(self, results: Dict, pca_results: Optional[Dict] = None, **kwargs):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle(
            'IVOM (Interpolated Variable Order Motifs) Analysis',
            fontsize=16,
            fontweight='bold',
        )
        ...
        axes[0, 1].scatter(
            x_positions, deviation_scores, c=colors, alpha=0.7, s=60
        )
    ...
    def plot_method_comparison(self, comparison_results: Dict, **kwargs):
        ...
        bars = axes[0, 0].bar(
            x_pos, detection_rates, color='skyblue', alpha=0.7, edgecolor='black'
        )
        ...
        axes[0, 0].set_xticklabels(
            [m.replace('_', ' ').title() for m in methods], rotation=45
        )
        axes[0, 0].grid(True, alpha=0.3)
                ...
                for j, method2 in enumerate(methods):
                    if i == j:
                        ...
                        if overlap_key in overlap_analysis:
                            jaccard_matrix[i, j] = overlap_analysis[overlap_key][
                                'jaccard_index'
                            ]
                        elif reverse_key in overlap_analysis:
                            jaccard_matrix[i, j] = overlap_analysis[reverse_key][
                                'jaccard_index'
                            ]
            ...
            if len(vote_distribution) > 0:
                ...
                for i, count in enumerate(vote_distribution):
                    if count > 0:
                        axes[1, 0].text(
                            i,
                            count + max(vote_distribution) * 0.01,
                            str(count),
                            ha='center',
                            va='bottom',
                            fontweight='bold',
        ...
        for method in methods:
            ...
            for other_method in methods:
                if other_method != method:
                    other_methods_anomalies.update(
                        np.where(method_predictions[other_method])[0]
                    )
    ...
    def plot_ivom_sliding_window(
        self, results: Dict, genome_data: Optional[Dict] = None, **kwargs
    ):
        """
        ...
        fig.suptitle(
            f'IVOM Sliding Window Analysis: {results.get("parent_sequence", "Unknown")}',
            fontsize=16,
            fontweight='bold',
        )
        ...
        anomaly_centers = [
            center for center, anomaly in zip(window_centers, is_anomaly) if anomaly
        ]
        anomaly_scores = [
            score for score, anomaly in zip(deviation_scores, is_anomaly) if anomaly
        ]
    ...
    def plot_results(
        ...
        **kwargs,
        ...
        if analysis_type == 'z_score':
            ...
            print(
                "Available types: 'z_score', 'clustering', 'isolation_forest', 'lof', 'one_class_svm', 'ivom', 'ivom_sliding_window', 'method_comparison'"
            )
