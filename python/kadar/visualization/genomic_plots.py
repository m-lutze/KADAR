...
class GenomicLocationVisualizer:
    ...
    def plot_genomic_overview(
        ...
        fig.suptitle(
            'Genomic Island Detection Overview', fontsize=16, fontweight='bold'
        )

        genome_length = len(genome_data['genome'])
        positions = [
            pos[0] + window_results['window_size'] // 2
            for pos in window_results['positions']
        ]

        # 1. Chromosome overview with predicted islands
        self._plot_chromosome_overview(
            axes[0], genome_data, window_results, predictions, genome_length
        )
        ...
        self._plot_compositional_features(
            axes[2], genome_data, window_results, positions, genome_length
        )
    ...
    def _plot_chromosome_overview(
        ...
        if 'is_anomaly' in predictions:
            ...
            window_results['step_size']

            for _i, (is_anomaly, pos) in enumerate(
                zip(predictions['is_anomaly'], window_results['positions'])
            ):
                if is_anomaly:
                    start, end = pos
                    ax.add_patch(
                        Rectangle(
                            (start, -chromosome_height / 2 - 0.15),
                            end - start,
                            0.15,
        ...
        legend_elements = [
            ...
            patches.Patch(
                color=self.colors['island_predicted'], label='Predicted Islands'
            ),
        ]
    ...
    def _plot_detection_scores(
        self, ax, positions: List[int], predictions: Dict, genome_length: int
    ):
        ...
        if 'anomaly_scores' in predictions:
            ...
            ax.scatter(positions, scores, c=colors, alpha=0.7, s=30)
    ...
    def _plot_compositional_features(
        self,
        ax,
        genome_data: Dict,
        window_results: Dict,
        ...
        # Plot GC content
        ax.plot(
            positions, gc_contents, 'g-', linewidth=2, alpha=0.8, label='GC Content'
        )
        ax.fill_between(positions, gc_contents, alpha=0.3, color='green')

        # Add overall genome GC content as reference line
        overall_gc = (
            genome_data['genome'].count('G') + genome_data['genome'].count('C')
        ) / len(genome_data['genome'])
        ax.axhline(
            y=overall_gc,
            color='darkgreen',
            linestyle='--',
    ...
    def plot_circular_genome(
        ...
        if 'annotations' in genome_data:
            for ann in genome_data['annotations']:
                ...
                ax.plot(
                    arc_angles,
                    [arc_radius] * len(arc_angles),
                    color=self.colors.get(ann['type'], 'red'),
                    linewidth=8,
                    alpha=0.8,
                    label=f'True {ann["type"]}',
                )

        # Plot predicted islands
        if 'is_anomaly' in predictions:
            for i, (is_anomaly, pos) in enumerate(
                zip(predictions['is_anomaly'], window_results['positions'])
            ):
                if is_anomaly:
        ...
        window_profiler = window_results['profiler']
        gc_contents = [
            window_profiler.calculate_gc_content(window_id)
            for window_id in window_results['windows']
        ]
        ...
        gc_normalized = [
            (gc - gc_min) / (gc_max - gc_min) * 0.3 + 0.4 for gc in gc_contents
        ]

        angles_gc = [pos_to_angle(pos) for pos in positions]
        ax.scatter(
            angles_gc, gc_normalized, c=gc_contents, cmap='RdYlGn', s=20, alpha=0.7
        )
    ...
    def plot_island_details(
        ...
        for _i, (island, ax) in enumerate(zip(island_regions, axes)):
            self._plot_single_island_detail(ax, genome_data, island, predictions)

        plt.tight_layout()
        plt.show()

    def _plot_single_island_detail(
        self, ax, genome_data: Dict, island: Dict, predictions: Dict
    ):
        ...
        if window_size > 0:
            ...
            for pos in range(plot_start, plot_end, window_size):
                ...
                if len(window_seq) > 0:
                    gc = (window_seq.count('G') + window_seq.count('C')) / len(
                        window_seq
                    )
        ...
        ax.set_title(
            f'{island["type"].title()} Island: {start:,} - {end:,} bp ({island_length:,} bp)'
        )
    ...
    def create_summary_map(
        self,
        multiple_genomes: Dict[str, Dict],
        ...
        max_length = max(
            len(genome_data['genome']) for genome_data in multiple_genomes.values()
        )
        ...
        for i, (genome_id, genome_data) in enumerate(multiple_genomes.items()):
            ...
            if 'positions' in predictions:
                for pos, is_anomaly in zip(
                    predictions['positions'], predictions['is_anomaly']
                ):
                    if is_anomaly:
                        start, end = pos
                        ax.add_patch(
...
def plot_genomic_locations(
    self, genome_data: Dict, window_results: Dict, predictions: Dict, **kwargs
):
    """
    ...
    if plot_type == 'overview':
        ...
        genomic_viz.plot_island_details(
            genome_data, kwargs['island_regions'], predictions
        )
    else:
        genomic_viz.plot_genomic_overview(genome_data, window_results, predictions)
...
def visualize_genomic_locations(
    self, genome_data: Dict, window_results: Dict, plot_type: str = 'overview', **kwargs
):
    """
    ...
    if 'isolation_forest' in self.results:
        ...
        print(
            'No isolation forest results available. Run isolation_forest_analysis first.'
        )
        return
    ...
    if plot_type == 'overview':
        ...
        island_regions = kwargs.get(
            'island_regions', genome_data.get('annotations', [])
        )
...
def example_genomic_visualization():
    ...
    """
    from genome_island_predictor import GenomeIslandPredictor, KmerProfiler
    ...
    window_results = predictor.sliding_window_analysis(
        'synthetic_genome', window_size=5000, step_size=2500
    )
    ...
    genomic_viz.plot_island_details(
        genome_data, genome_data['annotations'], predictions
    )
