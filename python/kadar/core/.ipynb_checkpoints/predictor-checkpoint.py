...
This module provides the GenomeIslandPredictor class that coordinates
various statistical analysis methods for genome island detection.
...
class GenomeIslandPredictor:
    """
    Main class for predicting genome islands using various statistical techniques.

    This class coordinates different analysis methods and manages results storage
    and visualization for genome island detection based on k-mer compositional
    characteristics.
    """
    ...
    def __init__(self, profiler: KmerProfiler):
        ...
        Args:
            profiler: Configured KmerProfiler instance
    ...
    def sliding_window_analysis(
        self, seq_id: str, window_size: int = 1000, step_size: int = 500
    ) -> Dict:
        """
        ...
        if len(sequence) < window_size:
            raise ValueError(
                f'Sequence length ({len(sequence)}) is smaller than window size ({window_size})'
            )
        ...
        temp_profiler = KmerProfiler(
            k=self.profiler.k, normalize=self.profiler.normalize
        )
    ...
    def clustering_analysis(
        self, seq_ids: Optional[List[str]] = None, method: str = 'dbscan', **kwargs
    ) -> Dict:
        """
    ...
    def isolation_forest_analysis(
        self, seq_ids: Optional[List[str]] = None, contamination: float = 0.1
    ) -> Dict:
        """
    ...
    def local_outlier_factor_analysis(
        self, seq_ids: Optional[List[str]] = None, n_neighbors: int = 20
    ) -> Dict:
        """
    ...
    def one_class_svm_analysis(
        self, seq_ids: Optional[List[str]] = None, nu: float = 0.1
    ) -> Dict:
        """
    ...
    def ivom_sliding_window_analysis(
        self,
        seq_id: str,
        window_size: int = 5000,
        step_size: int = 2500,
        reference_proportion: float = 0.1,
        **ivom_kwargs,
    ) -> Dict:
        """
    ...
    def run_comprehensive_analysis(
        ...
        comprehensive_results['z_score'] = self.z_score_analysis(
            reference_seqs, target_seqs
        )
        ...
        comprehensive_results['clustering'] = self.clustering_analysis(
            all_seqs, method='dbscan'
        )

        print('Running Isolation Forest...')
        comprehensive_results['isolation_forest'] = self.isolation_forest_analysis(
            all_seqs
        )

        print('Running chi-square analysis...')
        comprehensive_results['chi_square'] = self.chi_square_analysis(
            reference_seqs, target_seqs
        )
        ...
        try:
            comprehensive_results['ivom'] = self.ivom_analysis(
                reference_seqs, target_seqs
            )
        ...
        for analysis_type, results in self.results.items():
            try:
                if analysis_type == 'z_score':
                    summary[f'{analysis_type}_outliers'] = np.sum(
                        results['composite_z_scores'] > 2
                    )
                    summary[f'{analysis_type}_high_outliers'] = np.sum(
                        results['composite_z_scores'] > 3
                    )

                elif analysis_type in ['isolation_forest', 'lof', 'one_class_svm']:
                    summary[f'{analysis_type}_anomalies'] = np.sum(
                        results['is_anomaly']
                    )
                    summary[f'{analysis_type}_anomaly_rate'] = np.mean(
                        results['is_anomaly']
                    )

                elif analysis_type == 'clustering':
                    unique_labels = set(results['labels'])
                    summary[f'{analysis_type}_clusters'] = len(unique_labels) - (
                        1 if -1 in unique_labels else 0
                    )
                    summary[f'{analysis_type}_noise_points'] = np.sum(
                        results['labels'] == -1
                    )
                    if results.get('silhouette_score') is not None:
                        summary[f'{analysis_type}_silhouette'] = results[
                            'silhouette_score'
                        ]

                elif analysis_type == 'chi_square':
                    summary[f'{analysis_type}_significant'] = np.sum(
                        results['p_values'] < 0.05
                    )
                    summary[f'{analysis_type}_highly_significant'] = np.sum(
                        results['p_values'] < 0.001
                    )

                elif analysis_type == 'pca':
                    summary[f'{analysis_type}_components'] = len(
                        results['explained_variance_ratio']
                    )
                    summary[f'{analysis_type}_total_variance'] = np.sum(
                        results['explained_variance_ratio']
                    )
                elif analysis_type in ['ivom', 'ivom_sliding_window']:
                    if 'error' not in results:
                        summary[f'{analysis_type}_anomalies'] = results.get(
                            'n_anomalies', 0
                        )
                        summary[f'{analysis_type}_anomaly_rate'] = results.get(
                            'anomaly_rate', 0.0
                        )
                        summary[f'{analysis_type}_threshold'] = results.get(
                            'threshold', 0.0
                        )
                        summary[f'{analysis_type}_max_deviation'] = np.max(
                            results.get('deviation_scores', [0])
                        )
                        summary[f'{analysis_type}_mean_deviation'] = np.mean(
                            results.get('deviation_scores', [0])
                        )

                        if 'ivom_params' in results:
                            params = results['ivom_params']
                            summary[f'{analysis_type}_order_range'] = (
                                f'{params["min_order"]}-{params["max_order"]}'
                            )
                else:
    ...
    def compare_methods(self, methods: List[str] = None) -> Dict:
        ...
        for method in methods:
            if method in self.results and 'is_anomaly' in self.results[method]:
                comparison['method_predictions'][method] = self.results[method][
                    'is_anomaly'
                ]
        ...
        for i, method1 in enumerate(method_names):
            for _j, method2 in enumerate(method_names[i + 1 :], i + 1):
        ...
        if len(method_names) >= 2:
            all_predictions = np.array(
                [comparison['method_predictions'][method] for method in method_names]
            )
    ...
    def __repr__(self):
        ...
        return (
            f'GenomeIslandPredictor(profiler={repr(self.profiler)}, '
            f'analyses_run={list(self.results.keys())})'
        )
